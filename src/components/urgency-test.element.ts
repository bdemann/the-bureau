...
export const UrgencyTestElement = defineElement()({
    ...
    render() {
        ...
        return html`
            <h1>URGENCY GRID</h1>
            <p class="lede">
                Each row simulates one task. Each column simulates the value of
                "today" for that scenario. Cells show the daily-band the engine
                assigns. Verify boundaries and edge cases before shipping.
            </p>

            <div class="legend">
                <span class="swatch"
                    ><span class="box band-mandatory"></span>MANDATORY</span
                >
                <span class="swatch"
                    ><span class="box band-suggested"></span>SUGGESTED</span
                >
                <span class="swatch"
                    ><span class="box band-radar"></span>RADAR</span
                >
                <span class="swatch"
                    ><span class="box band-backlog"></span>BACKLOG</span
                >
                <span class="swatch"
                    ><span class="box band-hidden"></span>HIDDEN</span
                >
            </div>
            ...
            ${cases.map(
                (group) => html`
                    <h2
                        style="font-family:var(--font-display);margin:24px 0 6px;letter-spacing:0.08em;"
                    >
                        ${group.title}
                    </h2>
                    <table>
                                ...
                                (c) => html`
                                    <tr>
                                        <td class="label">
                                            ${c.name}
                                            ${c.note
                                                ? html`<small>${c.note}</small>`
                                                : html``}
                                        </td>
                                        ${offsets.map((d) => {
                                            const simToday = addDays(today, d);
                                            const band = getDailyBand(
                                                c.task,
                                                simToday,
                                            );
                                            return html`
