...
export const InsightsViewElement = defineElement<{
    ...
    styles: css`
        ...
        /* Areas overview table — scrolls horizontally on very narrow screens */
        .ops-table-wrap {
            overflow-x: auto;
    ...
    render({ inputs, dispatch, events }) {
        ...
        const { tasks, areas } = inputs;
        const areasById = new Map(areas.map((p) => [p.id, p]));
        ...
        const topStreaks = tasks
            ...
            .sort(
                (a, b) => b.maxTaskCompletionStreak - a.maxTaskCompletionStreak,
            )
            .slice(0, 5);
        ...
        function areaName(task: Task): string {
            if (!task.areaId) return "Unassigned";
            return areasById.get(task.areaId)?.name ?? "Unknown";
        }

        const renderMissedRow = (task: Task) => html`
            <div class="insight-row">
                <span class="badge badge-miss"
                    >${task.totalMisses > 1 ? task.totalMisses : ""} MISS</span
                >
                    ...
                    ? html`
                          <div class="missed-actions">
                              <button
                                  ...
                                  @click=${() =>
                                      dispatch(
                                          new events.missedTaskRevived(task.id),
                                      )}
                              >
                                  Revive
                              </button>
                              <button
                                  ...
                                  @click=${() =>
                                      dispatch(
                                          new events.missedTaskDismissed(
                                              task.id,
                                          ),
                                      )}
                              >
        ...
        const renderSnoozeRow = (task: Task) => html`
            <div class="insight-row">
                ...
                <div class="insight-meta">
                    <div class="insight-title">${task.title}</div>
                    <div class="insight-sub">${areaName(task)}</div>
                </div>
            </div>
        `;
        ...
        const num = (n: number) =>
            html`<span class="${n === 0 ? "zero" : ""}">${n}</span>`;
            ...
            <section class="section">
                <div class="section-header">
                    ...
                    <span class="section-count"
                        >${hardMissed.length + recurringMissed.length}</span
                    >
                </div>
                ${hardMissed.length === 0 && recurringMissed.length === 0
                    ? html`<div class="section-empty">
                          No missed commitments on record. Exemplary.
                      </div>`
                    : html`
                              ...
                              (task) => html`
                                  <div class="streak-row">
                                      ...
                                      <span class="streak-current"
                                          >${task.taskCompletionStreak}✓</span
                                      >
                                      <span class="streak-max"
                                          >best:
                                          ${task.maxTaskCompletionStreak}</span
                                      >
                                  </div>
                              `,
                          )}
                      ...
                      <section class="section">
                          <div class="section-header">
                              <span class="section-title"
                                  >RESPONSIBILITIES OVERVIEW</span
                              >
                          </div>
